# Hi, I'm Tejas Shah 👋
**AI/ML Engineer** | Deep Learning • NLP • Computer Vision • MLOps

I build end-to-end ML systems: from data + training to deployment, monitoring, and iteration.

---

## 👨‍💻 About me
- **Role**: AI/ML Engineer
- **Focus**: Model development, evaluation, productionization, and ML system design
- **Interests**: LLMs, retrieval (RAG), multi-modal ML, scalable inference

---

## 🧰 Tech stack
- **Languages**: Python, SQL, (Basics) C/C++
- **ML/DL**: PyTorch, TensorFlow/Keras, scikit-learn
- **NLP/LLMs**: Transformers, Hugging Face, RAG, Vector DBs
- **Data**: Pandas, NumPy, Spark (basics)
- **MLOps**: Docker, GitHub Actions, MLflow (basics), FastAPI
- **Cloud**: AWS/GCP (basics)

---

## 🚀 Featured projects
### 🎲 Snake & Ladder Game
- **What it is**: A fun Snake & Ladder game implementation (single-player / multiplayer / CLI / GUI — your choice).
- **Repo**: `https://github.com/mrtejasshah/<YOUR_SNAKE_AND_LADDER_REPO>`
- **Highlights**: Clean game logic, testable design, and optional UI.

> Tip: Once you have the game repo ready, replace the link above with your real repo URL and add a screenshot/GIF.

---

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=mrtejasshah&show_icons=true&hide_title=true)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=mrtejasshah&layout=compact)

---

## 🤝 Connect with me
- **LinkedIn**: `https://www.linkedin.com/in/<YOUR_HANDLE>/`
- **Email**: `tejasashah2004@gmail.com`

